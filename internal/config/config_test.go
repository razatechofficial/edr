...
func TestDefaults(t *testing.T) {
	...
	if cfg.XDR.SpoolMaxBytes != 3<<30 {
		t.Errorf("XDR.SpoolMaxBytes = %d, want 3GiB", cfg.XDR.SpoolMaxBytes)
	}
	if cfg.XDR.SpoolMaxAgeDays != 7 {
		t.Errorf("XDR.SpoolMaxAgeDays = %d, want 7", cfg.XDR.SpoolMaxAgeDays)
	}
...
func TestLoadXDREnabledYAMLWithoutEnv(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("DATA_DIR", dir)
	_ = os.Unsetenv("XDR_ENABLED")
	_ = os.Unsetenv("XDR_SECURE_STORAGE")
	path := filepath.Join(dir, "agent.yaml")
	raw := "agent:\n  id: a1\n  data_dir: " + dir + "\nxdr:\n  enabled: true\n  secure_storage: file\n  enrollment_host: enroll.xdr.averox.com:443\n"
	...
	if !cfg.XDR.Enabled {
		t.Fatal("yaml xdr.enabled:true must survive unset XDR_ENABLED")
	}
