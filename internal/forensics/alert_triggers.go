...
func CollectArtifactsForAlert(ctx context.Context, log *zap.Logger, meta AlertTriggerMeta) (*ArtifactBundle, error) {
	if log == nil {
		log = zap.NewNop()
	}
	...
	defer cancel()
	return NewArtifactCollector(log).CollectAll(ctx)
}
