...
/*
 * Windows WDM Kernel Driver for EDR Agent.
 * Provides:
 *   - Process handle protection via ObRegisterCallbacks
 *   - IOCTL interface for managing the protected PID list from userspace
 *
 * Build: compiled as a WDM driver using the Windows Driver Kit (WDK).
 */

#include <ntddk.h>
#include <wdm.h>

#define EDR_DEVICE_NAME  L"\\Device\\EdrAgent"
#define EDR_SYMLINK_NAME L"\\DosDevices\\EdrAgent"
...
typedef struct _EDR_CONTEXT {
	HANDLE              ProtectedPids[MAX_PROTECTED_PIDS];
	...
	_Inout_ POB_PRE_OPERATION_INFORMATION OpInfo)
{
	...
	/* Strip dangerous access rights from handles to protected processes. */
	...
	_Inout_ PIRP Irp)
{
	...
	case IOCTL_EDR_ADD_PROTECTED_PID: {
		...
		ExAcquireFastMutex(&g_Ctx.PidLock);
		if (g_Ctx.ProtectedPidCount < MAX_PROTECTED_PIDS) {
